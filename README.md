...
* **MLOps 및 AI 신뢰도 최적화:**
  * CPU 환경의 연산 병목 및 타임아웃 방지를 위해 원문 분석을 최대 5개 조각(2,500자)으로 제한하는 **Truncation(샘플링) 기법**을 적용하여 추론 속도를 최적화했습니다.
  * '증권발행실적' 등 의례적 위험 문구가 포함된 정상 공시를 사전 필터링하여 **False Positive(오탐지)**를 방지했습니다.
